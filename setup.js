...
const start = async () =>{
  const del = await new Promise(resolve => {
    rl.question("Setup db? (Y/n) ", resolve)
  })
  if (del.toLowerCase() == 'n') {rl.close(); return;}
  if (del.toLowerCase() == 'y' || del == ''){
    const name = await new Promise(resolve => {
      rl.question("Admin name? ", resolve)
    })
    const login = await new Promise(resolve => {
      rl.question("Admin login? ", resolve)
    })
    const password = await new Promise(resolve => {
      rl.question("Admin password? ", resolve)
    })

    fs.unlink('todo.db', (error) => {
      if (error) {console.error('No todo.db')} // если возникла ошибка
    })

    fs.copyFileSync('new.db', 'todo.db')
    ...
    let _sql = `INSERT INTO users(login,password,name,ID_role,hash) VALUES(?,?,?,?,?)`
            db.get(_sql, [login,password,name,1,hash], (err) => {
                if (err) {
                    console.error(err);
                    rl.close();
                    return
